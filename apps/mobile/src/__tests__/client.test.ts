...
import { ConfigError, parseApiBaseUrl } from '../config';
import { fakeTransport } from './support';
...
describe('client', () => {
  it('sends the bearer header only when a token is supplied', async () => {
    ...
    const request = createClient('http://api.test', http.transport);
  ...
  it('surfaces the API validation shape', async () => {
    ...
    const result = await createClient('http://api.test', http.transport)(
      '/auth/register',
      { method: 'POST', body: {} },
    );
  ...
  it('reports a transport failure as offline, not as a status', async () => {
    const request = createClient('http://api.test', async () => {
...
describe('parseApiBaseUrl (AC-2)', () => {
  it('accepts a LAN origin and trims the trailing slash', () => {
    expect(parseApiBaseUrl('http://192.168.1.10:3000/')).toBe(
      'http://192.168.1.10:3000',
    );
  });

  it('names the variable when unset', () => {
    expect(() => parseApiBaseUrl(undefined)).toThrow(ConfigError);
    expect(() => parseApiBaseUrl(undefined)).toThrow('EXPO_PUBLIC_API_URL');
  });

  it('rejects an empty value rather than treating it as a relative origin', () => {
    expect(() => parseApiBaseUrl('   ')).toThrow(ConfigError);
  });

  it('rejects a value that is not an absolute http(s) URL', () => {
    expect(() => parseApiBaseUrl('192.168.1.10:3000')).toThrow(ConfigError);
    expect(() => parseApiBaseUrl('ftp://192.168.1.10')).toThrow(ConfigError);
  });
});
