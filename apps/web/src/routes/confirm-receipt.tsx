...
type ItemFields = { description: string; quantity: string; unitPrice: string; lineTotal: string };
const emptyItem = (): ItemFields => ({ description: '', quantity: '', unitPrice: '', lineTotal: '' });
...
function validateItems(items: ItemFields[]): { values?: ExpenseItem[]; errors: Record<string, string> } {
  ...
  const values = items.flatMap((item, index) => {
    ...
    if (unitPriceCents === undefined) errors[`items.${index}.unitPriceCents`] = 'Use a valid amount with no more than 2 decimal places.';
    if (lineTotalCents === undefined) errors[`items.${index}.lineTotalCents`] = 'Use a valid amount with no more than 2 decimal places.';
    if (!description && !quantity && unitPriceCents === null && lineTotalCents === null) return [];
...
export function ConfirmReceiptScreen() {
  ...
  function changeItem(index:number,name:keyof ItemFields,value:string){setItems((current)=>current.map((item,i)=>i===index?{...item,[name]:value}:item));setErrors((current)=>{const next={...current};delete next[`items.${index}.${name === 'unitPrice' ? 'unitPriceCents' : name === 'lineTotal' ? 'lineTotalCents' : name}`];return next})}
  ...
  const lineTotals=items.map((item)=>item.lineTotal.trim()===''?undefined:decimalToCents(item.lineTotal));
  const hasMismatch=items.length>0&&reconciliationTarget!==undefined&&lineTotals.every((value)=>value!==undefined)&&lineTotals.reduce((sum,value)=>sum+value!,0)!==reconciliationTarget;
