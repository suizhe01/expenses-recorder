...
export function createSessionManager({
  ...
  async function rotate(presented: string): Promise<RefreshOutcome> {
    // The SAME token is presented on every 503 retry, and that is correct
    // rather than an oversight — it reads like the thing that trips theft
    // detection, so it is worth stating plainly.
    //
    // A 503 is the server saying another rotation of this session is already
    // in flight, which means *this* one did not happen: the token is unspent
    // and re-presenting it is the only thing that could work. It is never
    // this client racing itself, because refresh is single-flight (AC-5), and
    // any other status is terminal and returns below without retrying.
    for (let attempt = 0; attempt <= REFRESH_503_RETRIES; attempt += 1) {
      const result = await auth.refresh(presented);
