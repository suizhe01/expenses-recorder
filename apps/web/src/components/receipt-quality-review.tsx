...
function Review({ file, onUse, onRetake }: { file: File; onUse: (file: File) => void; onRetake: () => void }) {
  ...
  const [currentFile, setCurrentFile] = useState(file); const [preview, setPreview] = useState(() => URL.createObjectURL(file)); const [available, setAvailable] = useState(false); const [warnings, setWarnings] = useState<Warning[]>([]);
  ...
  function resetDraft() { setCrop({ x: 0, y: 0, width: 1, height: 1 }); setRotation(0); setError(undefined); }
  async function apply() { if (!image.current) return; try { setError(undefined); const next = await jpegFrom(image.current, crop, rotation); if (next.size > MAX_BYTES) { setError('The edited photo is larger than 10 MB. Crop further, reset, or retake it.'); return; } setCurrentFile(next); setPreview(URL.createObjectURL(next)); setAvailable(false); setWarnings([]); resetDraft(); setEditing(false); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not create the edited photo.'); } }
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onRetake(); }}><DialogContent className="max-h-[90dvh] overflow-y-auto"><DialogHeader><DialogTitle>{editing ? 'Edit photo' : 'Check receipt photo'}</DialogTitle><DialogDescription>{editing ? 'Drag across the image to choose a crop. Rotate is clockwise.' : 'Check the photo before it is uploaded and read.'}</DialogDescription></DialogHeader>
    ...
    <DialogFooter>{editing ? <><Button type="button" variant="outline" onClick={resetDraft}>Reset</Button><Button type="button" variant="outline" onClick={() => { resetDraft(); setEditing(false); }}>Cancel</Button><Button type="button" onClick={() => void apply()} disabled={crop.width < 0.05 || crop.height < 0.05}>Apply edits</Button><Button type="button" variant="outline" onClick={() => setRotation((value) => (value + 90) % 360)}>Rotate 90°</Button></> : <><Button type="button" variant="outline" onClick={onRetake}>Retake</Button>{available && <Button type="button" variant="outline" onClick={() => setEditing(true)}>Edit photo</Button>}<Button type="button" onClick={() => onUse(currentFile)}>{warnings.length ? 'Use anyway' : 'Use photo'}</Button></>}</DialogFooter>
