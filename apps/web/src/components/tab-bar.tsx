...
export function TabBar({ active, capture = true }: { active: 'overview' | 'expenses' | 'inbox'; capture?: boolean }) {
  ...
  async function choose(file: File) {
    ...
    if (result.kind === 'ok') {
      ...
      if (result.kind === 'offline') { setRetryFile(file); setNotice('Could not reach the server. Check your connection.'); }
  ...
  return <><div className="fixed inset-x-4 bottom-[calc(5rem+env(safe-area-inset-bottom))] z-40 mx-auto max-w-xl text-center text-sm" role={notice ? 'alert' : undefined} aria-live="polite">{uploading ? <span className="inline-flex items-center gap-2"><LoaderCircle className="size-4 animate-spin" />Reading your receipt…</span> : notice}{retryFile && <div className="mt-2 flex justify-center gap-2"><Button onClick={() => void choose(retryFile)}>Retry</Button><Button variant="outline" onClick={() => { setRetryFile(undefined); setNotice(undefined); if (input.current) input.current.value = ''; }}>Discard</Button></div>}</div><nav aria-label="Main navigation" className="fixed inset-x-0 bottom-0 z-30 h-[calc(4rem+env(safe-area-inset-bottom))] border-t bg-background/95 px-4 pb-[calc(0.5rem+env(safe-area-inset-bottom))] pt-2 backdrop-blur dark:border-border"><div className="relative mx-auto grid max-w-xl grid-cols-2 gap-2">{tabs.map(({ to, label, icon: Icon }) => { const isActive = (active === 'overview' || active === 'inbox') ? to === CLIENT_ROUTES.home : to === CLIENT_ROUTES.expenses; return <Link key={to} to={to} aria-current={isActive ? 'page' : undefined} className={cn('flex min-h-11 items-center justify-center gap-2 rounded-lg text-sm font-medium', isActive ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted dark:hover:bg-muted/70')}><Icon className="size-4" aria-hidden="true" />{label}</Link>; })}{capture && <label className="absolute left-1/2 top-0 flex size-14 -translate-x-1/2 -translate-y-5 cursor-pointer items-center justify-center rounded-full border-4 border-background bg-primary text-primary-foreground shadow-lg focus-within:ring-3 focus-within:ring-ring/50" aria-label="Add receipt"><CirclePlus className="size-7" aria-hidden="true" /><input ref={input} className="sr-only" type="file" accept="image/*" onClick={(event) => { event.currentTarget.value = ''; }} onChange={(event) => { const file = event.target.files?.[0]; if (file) void choose(file); }} /></label>}</div></nav></>;
}
