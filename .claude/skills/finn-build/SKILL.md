...
Using the Linear connector, list issues on team `EXP` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `EXP-NNN-short-slug`, using the issue's real identifier.
...
- `Closes EXP-NNN`, using the real Linear issue identifier
